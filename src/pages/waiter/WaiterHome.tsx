...
export default function WaiterHome() {
  ...
  async function handleTableTap(t) {
    if (t.status === 'available') {
      navigate(`/waiter/order/new?table=${t.id}&tableName=${t.name}`)
    } else if (t.status === 'occupied') {
      // Find existing open order for this table — add items to it
      ...
      if (existing) {
        navigate(`/waiter/order/${existing.id}?table=${t.id}&tableName=${t.name}`)
      } else {
        navigate(`/waiter/order/new?table=${t.id}&tableName=${t.name}`)
      }
    } else {
    ...
    <div style={{ minHeight:'100vh', background:'#f9fafb', fontFamily:'-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif' }}>
      ...
      <div style={{ background:'#059669', padding:'14px 16px', position:'sticky', top:0, zIndex:10 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ color:'white', fontWeight:700, fontSize:15 }}>{profile?.full_name || 'Waiter'}</div>
          ...
          <button onClick={async () => { await signOut(); navigate('/login') }} style={{ background:'rgba(255,255,255,0.2)', border:'none', borderRadius:8, padding:8, cursor:'pointer', color:'white' }}>
            <LogOut size={16} style={{ display:'block' }} />
          </button>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:12 }}>
          {[['Available',counts.available,'#bbf7d0','#14532d'],['Occupied',counts.occupied,'#fecaca','#7f1d1d'],['Reserved',counts.reserved,'#fde68a','#78350f']].map(([l,v,bg,tc]) => (
            <div key={l} style={{ background:bg, borderRadius:10, padding:'8px 10px', textAlign:'center' }}>
              ...
              <div style={{ fontSize:10, color:tc, opacity:0.8 }}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:16 }}>
        {loading ? (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {Array(8).fill(0).map((_,i) => <div key={i} style={{ height:110, background:'#e5e7eb', borderRadius:14 }} />)}
          </div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {filtered.map(t => {
