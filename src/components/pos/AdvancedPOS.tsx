...
export function MergeTables({ currentOrderId, onClose }: { currentOrderId: string; onClose: () => void }) {
  ...
  async function merge() {
    ...
    for (const orderId of selected) {
      await supabase.from('order_items').update({ order_id: currentOrderId }).eq('order_id', orderId)
...
export function TransferTable({ orderId, onClose }: { orderId: string; onClose: () => void }) {
  ...
  async function transfer() {
    ...
    await supabase.from('orders').update({ table_id: selected }).eq('id', orderId)
    await supabase.from('dining_tables').update({ status: 'occupied' }).eq('id', selected)
...
export function RefundVoid({ billId, onClose }: { billId: string; onClose: () => void }) {
  ...
  async function process() {
    ...
    setLoading(true)
    const { error } = await supabase.from('bills').update({ status: type === 'void' ? 'cancelled' : 'refunded' }).eq('id', billId)
